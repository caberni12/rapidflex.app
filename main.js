...
async function verificarSesion() {
  ...
  try {
    const response = await fetch(`${GAS_URL}?checkSession=1&session=${encodeURIComponent(token)}`);
    ...
    if (resultado.status === "OK") {
      ...
      } catch (_) { _setUserNameFromStorage(); }
    } else {
...
function _autoSizeIframe(frame) {
  ...
  try {
    ...
    if (frame._observer) { try { frame._observer.disconnect(); } catch (_) {} }
...
const ROUTES = {
  ...
  "georeferencia": () => loadFrame("usuarios2.html") // alias si lo usas en el menú
};
...
function _envInfo() {
  try {
    ...
    const isPrivate = (
      ...
      /^172\.(1[6-9]|2[0-9]|3[0-1])\./.test(h) || !h.includes(".")
    );
    return { mode: isPrivate ? "private" : "public" };
  } catch (_) { return { mode: "unknown" }; }
}
